...
const ROUTE_CONFIG = {
    ...
    admin: {
        exactRoutes: [
            ...
            {path: '/admin/contact', methods: ['GET']},
            {path: '/admin/contact/stats', methods: ['GET']},
        ],
        patterns: [
            ...
            {pattern: /^\/admin\/contact\/\d+$/, methods: ['GET', 'PATCH', 'DELETE']},
            {pattern: /^\/admin\/contact\/\d+\/read$/, methods: ['PATCH']},
        ]
    }
}
...
async function request(path, options = {}) {
    ...
    if (routeType === 'public' && method === 'POST' && (path === '/reviews' || path === 'contact')) {
        try {
            await getCsrfCookie()
        } catch (csrfErr) {
